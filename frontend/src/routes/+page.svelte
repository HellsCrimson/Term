<script lang="ts">
  ...
  LoggingService.Log('Page called', "INFO");
  ...
  onMount(() => {
    ...
    (async () => {
      ...
      console.log('Registering keyboard shortcuts on document');
  ...
  function handleKeyDown(e: KeyboardEvent) {
    ...
    if (e.ctrlKey && e.key === 't') {
      ...
      if (selectedNode && selectedNode.type === 'session' && selectedNode.sessionType) {
        LoggingService.Log(`Creating tab for ${selectedNode.name}`, "INFO");
        terminalsStore.createTab(selectedNode.id, selectedNode.name, selectedNode.sessionType);
      } else {
        LoggingService.Log('No valid session selected', "INFO");
      }
      return;
    }
    ...
    // Ctrl+N: New session dialog (TODO: implement new session dialog)
